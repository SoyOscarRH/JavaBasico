...
public class Date{
    ...
    private void FixInput(){                                    //======= FUN FUNCTION ============
        if (day > 31) {day = 1; month++;}                       //If you stupid put it wrong
        if (month > 12) {month = 1; year++;}                    //If you stupid put it wrong
        if (year > 0 && year < 18) year += 2000;                //If you are refering to the XXI century
        else if (year > 0 && year < 16) year += 1900;           //If you are refering to the XIX century
    }

    public int toInt(){
        String Temporal = String.valueOf(day) + String.valueOf(month)+ String.valueOf(year);
        return parseInt(Temporal);
    }
